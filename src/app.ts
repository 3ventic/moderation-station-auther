...
const FOLLOWS_REQUIRED: number =
  (process.env.FOLLOWS_LIMIT && parseInt(process.env.FOLLOWS_LIMIT, 10)) ||
  15000;
const PARTNERS_REQUIRED: number =
  (process.env.PARTNERS_LIMIT && parseInt(process.env.PARTNERS_LIMIT, 10)) || 1;
...
const exchangeCode: (
  ...
  try {
    const r: FetchResponse = await fetch(
      `${CODE_EXCHANGE_BASE_MAP[service]}/oauth2/token`,
      {
        method: "POST",
...
) => Promise<HelixUser | null> = async (session: Express.Session) => {
  try {
...
const modTotals: (
  login: string
) => Promise<ModLookupUserTotals | null> = async (login: string) => {
  try {
    const r: FetchResponse = await fetch(
      `https://modlookup.3v.fi/api/user-totals/${encodeURIComponent(login)}`,
      {
        headers: {
...
const discordUser: (
  session: Express.Session
) => Promise<Discord.User | null> = async (session: Express.Session) => {
  try {
    ...
    if (await exchangeCode(req, res, exchange, "twitch")) {
      ...
      if (tUser === null) {
        ...
        if (mt === null) {
          ...
          mt.follows >= FOLLOWS_REQUIRED &&
          mt.partners >= PARTNERS_REQUIRED
        ) {
          if (
            await grantRoles(
              req.session!,
